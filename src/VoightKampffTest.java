...
public class VoightKampffTest {
        ...
        while(toParse.size() > 1){
            ...
            for(int i = 0; i < toParse.size(); i++){
                if(toParse.get(i).equals("(")){
                    ...
                    toParse.set(i, calculate(subCharList)); // reassign values
                    for(int j = 1; j < subCharList.size()+2; j++){
                        toParse.remove(i-j);
                    }
                    return(innerPlTrue(toParse, value)); // converting to string ruins it partially
                }
                if(!toParse.get(i).equals("(")){
    ...
    public boolean plTrue(String toParse, boolean[] value) {
        ...
        toParse = toParse.replace(" ", ""); // get rid of empty spaces in string

        ArrayList<Object> charList = new ArrayList<>(Arrays.asList(toParse.split(""))); // split string into list of chars
        //   ^^^ Unsafe in production code, realistically we'd want a wrapper to accept only Bool and String/Char

        ArrayList<String> usedChars = new ArrayList<>(); // list of used chars - Initialized as empty list

        for (int i = 0; i < charList.size(); i++) { // for each char in the list
            String tempHolder = (String) charList.get(i);
            if(tempHolder.contentEquals(tempHolder.toLowerCase()) && tempHolder.matches("[^v>^()]")) { // Regex excluding common symbols
        ...
        while(charList.size() > 1){
            ...
            for(int i = 0; i < charList.size(); i++){
                if(charList.get(i).equals("(")){
                    ...
                    charList.set(i, calculate(subCharList)); // reassign values
                    for(int j = 1; j < subCharList.size()+2; j++){
                        charList.remove(i-j);
                    }
                    return(innerPlTrue(charList, value)); // converting to string ruins it partially
                }
                if(!charList.get(i).equals("(")){
